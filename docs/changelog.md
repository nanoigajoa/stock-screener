...
## [2026-05-07] 현직자 피드백 기반 지표 4종 추가

### 배경

퀀트 엔지니어링 관점 코드 리뷰 결과, 기존 시스템이 "후행성 지표 + 시간축 거래량" 에 편중된다는 사각지대 진단. 가격 구조(Price Action)와 유동성 매트릭스 관점의 지표 4종 추가.

### 구현 내용

**1. FVG (Fair Value Gap) — `signal_scorer.py`, `buy_signal.py`**
- SMC(스마트 머니 컨셉) 기반 3봉 상승 갭: `Low[t] > High[t-2]`
- 현재가가 FVG 구간(zone_bot ~ zone_top × 1.02) 내 → Pattern 카테고리 +0.9점
- buy_signal.py에서도 FVG 지지 반등을 별도 진입 조건으로 추가

**2. ADX(30) — `signal_scorer.py`**
- 추세 강도 지표 (방향 무관): ADX < 20 횡보 → ADX > 50 강추세
- Momentum 카테고리 보조: `min(max((adx-20)/30, 0), 1)` 연속값

**3. Volume Profile POC — `screener/indicators.py`, `signal_scorer.py`**
- 20일 가격축 거래량 분포 → 최다 밀집 가격(POC) 계산
- 현재가가 POC ~ POC×1.02 직상방 진입 시 Pattern +0.8점 (기관 평단가 지지 확인)

**4. Max Drawdown Gate — `screener/checklist.py`**
- 60일 최고가 대비 -20% 초과 하락 시 hard skip (데드캣 바운스 필터)
- RSI 하드게이트보다 앞 순서로 적용

**추가: RSI Hysteresis — `signal_scorer.py`**
- 최근 10봉 내 RSI < 30 방문 후 현재 RSI < 35 미회복 → momentum_score × 0.5
- 데드캣 바운스 억제 (과매도 회복 확인 전 진입 차단)

**추가: Chandelier Exit Long — `screener/sell_signal.py`**
- `highest_22 - ATR(22) × 3` 이탈 시 매도 마커 "샹들리에이탈"
- 4번째 매도 마커 유형으로 추가
...
기존 Trend(35%) / Momentum(25%) / Volume(25%) / Pattern(15%) 구조의 구조적 문제 진단:
- CMF·OBV·MFI 모두 동일 정보(수급 방향)를 반복 측정 → Volume 카테고리 실질 가중치 과대
- STRONG BUY 기준이 "비슷한 종류의 지표들이 동시에 켜지면" 달성 가능 → 신뢰도 낮음
- 진짜 강한 신호는 "서로 다른 정보 계층"이 교차할 때

### 새 아키텍처: 3개 독립 정보 계층

| 축 | 가중치 | 핵심 지표 | 측정 대상 |
|----|:---:|---------|---------|
| 진입 포지션 | 40% | ATR 진입존 + BB %B | 가격 위치 |
| 모멘텀 강도 | 35% | RSI 종형 + ADX | 가격 속도·추세 강도 |
| 구조 확인 | 25% | LiquiditySweep·FVG·POC + 캔들 패턴 | 기관 흔적 |
| 수급 보너스 | +5% | CMF + OBV 이중 확인 | 교차 검증 (가산) |

**핵심 변경:**
- 수급(CMF·OBV)을 독립 카테고리에서 교차 검증 보너스로 강등 → 상관 지표 중복 제거
- Structure 카테고리 신설 (FVG·POC·LiquiditySweep) — 가격축 기반 독립 정보
- STRONG BUY 달성은 구조 확인(0.25축) 없이는 수학적으로 거의 불가능 (신뢰도 설계)

### 변경 파일

| 파일 | 변경 내용 |
|------|----------|
| `screener/signal_scorer.py` | 전면 재작성 — 3축 스코어링, confluence 계산 |
| `services/signal_service.py` | SPY MA50 Regime Filter 추가 |
| `templates/partials/signal_cards.html` | breakdown 키 동기화 (entry/momentum/structure/volume) |
| `static/js/signals.js` | CAT_LABELS 키 동기화 |
| `api/routes/explore.py` | _CAT_LABELS 키 동기화 |
| `screener/nl_generator.py` | breakdown 키 동기화 |

**SPY MA50 Regime Filter:**
- `services/signal_service.py`에서 SPY OHLCV 함께 수집
- SPY 종가 < MA50: 하락장 판정 → 모든 STRONG BUY·BUY 결과 WATCH로 캡핑
- `summary.spy_regime` bool 반환, 하락장 시 UI 경고 배너 표시
...
STRONG BUY 등급이 나와도 "어떤 근거로 신뢰해야 하는가"에 대한 답이 없었음.
점수를 높이는 것과 신뢰도를 표시하는 것은 다른 문제 — Confluence Check는 점수를 건드리지 않고 신호 품질을 별도 레이어로 노출.

### 3개 독립 레이어

| 레이어 | 조건 | 독립성 근거 |
|--------|------|------------|
| 구조 지지 | FVG·POC·LiquiditySweep 중 1개 이상 | 가격축 기반 |
| 모멘텀 회복 | RSI 35~60 (과매도 회복 확인, 과매수 전) | 시간축 오실레이터 |
| 자금 유입 | CMF > 0 (매수 압력 양수) | 거래량×가격 합성 |

### 구현 내용

- `screener/signal_scorer.py`: confluence_count(0~3), confluence_layers, confluence_detail 반환 추가
- `templates/partials/signal_cards.html`: 카드에 dots(●/○) 표시 — 활성 레이어만 색상 점, 3/3 시 CONFLUENCE 배지
- `static/js/signals.js`: 모달 하단 CONFLUENCE 패널 — 각 레이어 통과/실패 사유(RSI값, CMF값, 패턴명) 표시
- `static/css/signals.css`: `.confluence-row`, `.cf-dot`, `.msd-confluence` 스타일 추가
...
| 라우트 | 변경 전 | 변경 후 |
|--------|---------|---------|
| `/` | `/signals` 리다이렉트 | `/about` 리다이렉트 |

처음 접속하는 사용자에게 시스템 개요를 먼저 보여주도록 변경.

### 2. Signals 페이지 — NO SIGNAL 필터 버튼 추가

`sig-filter-bar`에 `– NO SIGNAL` pill 버튼 추가. 기존 ALL / STRONG BUY / BUY / WATCH 4개 → 5개.
`base.css`에 `.pill.is-active.tone-mute` 스타일 추가.

### 3. Signals 페이지 — 분석 완료 시각 동적 표시

`page-subtitle`의 "30초 전 갱신" 하드코딩 텍스트 → `<span id="signal-updated">` 로 교체.
SSE `done` 이벤트 수신 시 JavaScript로 `HH:MM 기준` 포맷 실시간 갱신.

### 4. Signals 페이지 — Watchlist 빈 상태 안내

Watchlist가 비어 있을 때 "종목 없음 — Finviz 전체 유니버스 상위 50개로 자동 분석합니다" 힌트 표시.
서버사이드(Jinja2 초기 렌더)와 클라이언트사이드(`_syncWatchlistTags`) 양쪽 처리.
`base.css`에 `.wl-empty-hint` 스타일 추가.

### 5. 전역 키보드 단축키 ⌘1 / ⌘2 / ⌘3

`base.html` 하단 인라인 스크립트로 네비게이션 단축키 바인딩.
...
기존 사이드바에 표시만 되고 실제로 동작하지 않던 문제 해결.

### 6. 스크리닝 카드 ★ 버튼 — filled star + pop 애니메이션

| 항목 | 변경 |
|------|------|
| 추가된 상태 | SVG `stroke`만 → `fill: currentColor` 추가 (별이 채워짐) |
| 클릭 시 | 0.25s `wl-pop` keyframe 애니메이션 (scale 1→1.35→1) |

`screen.css`에 `@keyframes wl-pop` + `.watchlist-btn.wl-pop` 추가.
`screen.js` click 핸들러에서 `classList.remove/add('wl-pop')` + `offsetWidth` reflow 트리거.

### 7. 시그널 카드 STOP 가격 표시

`signal_cards.html`의 ENTRY 존 아래에 `STOP $XX.XX` 행 추가.
`signal_stop` 값이 있는 카드에만 렌더링 (조건부).
`signals.css`에 `.sig-zone.sig-stop .val { color: var(--bear); }` 추가.

### 8. About 네비게이션 아이콘 교체

시계 모양 SVG (`M7 4V7L9 8.5` 시침 경로) → 정보 아이콘 (원 + 점 + 세로선 i 구조).

### run.sh 오타 수정

`# 1. Kill exi` + `sting process on port 8000` (줄 분리 오타) → `# 1. Kill existing process on port 8000`.
